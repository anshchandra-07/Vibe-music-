...
const PlayerControls = () => {
  ...
  const renderRepeatIcon = () => {
    switch (repeatMode) {
      case 'one':
        return <Repeat1 className="w-5 h-5 text-white" aria-hidden="true" />;
      case 'all':
        return <Repeat className="w-5 h-5 text-white" aria-hidden="true" />;
      case 'none':
      default:
        return <Repeat className="w-5 h-5 text-zinc-500 hover:text-zinc-300" aria-hidden="true" />;
    }
  };
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-5 select-none">
      ...
      <button
        ...
        className="p-2 rounded-lg hover:bg-white/5 transition-all focus:outline-none touch-target"
      ...
      <button
        onClick={togglePlay}
        disabled={isLoading}
        className="w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center bg-white text-black hover:scale-105 active:scale-95 transition-all shadow-lg focus:outline-none relative group touch-target"
        style={{ 
          boxShadow: isPlaying 
      ...
      >
        ...
        {isPlaying ? (
          <Pause className="w-5 h-5 sm:w-6 h-6 fill-black" aria-hidden="true" />
        ) : (
          <Play className="w-5 h-5 sm:w-6 h-6 fill-black translate-x-[1px]" aria-hidden="true" />
        )}
      </button>
